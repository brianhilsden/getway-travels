...
function LandingPage() {
  ...
  const [dataToDispay,setColor,setData,setShowSearchBar,setComponent, ,sortChoice,setSortChoice] = useOutletContext() // Destructuring context values via useOutletContext

  useEffect(()=>{ 
    setColor(prev=>({...prev,nav:"transparent",text:"white",lineargradient:""})) // Set color context attributes
...
function addToWishlist(packageSaved){
  ...
  if(packageSaved.wishlist === false){ // Check if the package is not in the wishlist
  ...
  .then(res=>res.json())
  .then(()=>setData(prevData=>prevData.map(item=>item.id === packageSaved.id?{...item,wishlist:!item.wishlist}:item))) // Update the local data to reflect wishlist change
  ...
  return (
    <div style={{ background: "whitesmoke" }}> 
      ...
      <div className="d-flex flex-wrap justify-content-center"> 
        {dataToDispay.map((data,index) => ( // Map over dataToDisplay to render cards
          <div
          key={index}
            className="card"
            style={{
