...
const SpecificPage = () => {
  ...
  const handleBookNow = () => {
    if (loggedIn) {
      alert(
        "You shall be contacted soon via email for confirmation and payment details"
      );
  ...
  useEffect(() => {
    fetch(
      `https://getway-travels-vercel-riqc7b6s7-brianhilsdens-projects.vercel.app/packages/${specificPageid}`
    )
      .then((res) => res.json())
  ...
  function addToWishlist(packageSaved) {
    if (packageSaved.wishlist === false) {
      ...
      fetch(
        "https://getway-travels-vercel-riqc7b6s7-brianhilsdens-projects.vercel.app/wishlist",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      fetch(
        `https://getway-travels-vercel-riqc7b6s7-brianhilsdens-projects.vercel.app/wishlist/${packageSaved.id}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <div
      ...
      style={{
        backgroundImage: `url('https://images.unsplash.com/photo-1471922694854-ff1b63b20054?q=80&w=1472&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90oy1wYWdlfHx8fGVufDB8fHx8fA%3D%3D')`,
    ...
    >
      ...
      <div className="d-flex align-items-center">
        <img
          src={SpecificPage.image}
          width="100%"
          height="50vh"
          className="ms-2"
          alt="Travel destination"
          style={{ top: 0, left: 0, order: -1, width: "100%", height: "50vh" }}
        />
      ...
      >
        ITINERARY: {SpecificPage.itinerary}
      </p>
      ...
      <button
        ...
        onClick={() => addToWishlist(SpecificPage)}
        style={{ marginRight: "10px" }}
      >
        {SpecificPage.wishlist ? "Remove from Wishlist" : "Add to Wishlist"}
      </button>
    </div>
  );
};
